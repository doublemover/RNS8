...
def validate_bounded_contract(self, ctx: dict[str, Any]) -> None:
    ...
    if self._is_direct_hip_bounded_skinny_gemv_small_n_capture():
        if bound_mode != "global":
            self._error("direct-HIP skinny GEMV small-N captures must use bound_mode=global")
        if residue_chain_length != 1 or residue_output_mode != "host_export":
            self._error("direct-HIP skinny GEMV small-N captures must use host_export residue_chain_length=1")
        expected_kernel = DIRECT_HIP_BOUNDED_SKINNY_GEMV_SMALL_N_KERNELS[semantics]
        expected_epilogue = DIRECT_HIP_BOUNDED_SKINNY_GEMV_SMALL_N_EPILOGUE
        if self.data.get("selected_kernel") != expected_kernel:
            self._error(
                "direct-HIP skinny GEMV small-N captures must use "
                f"selected_kernel={expected_kernel}"
            )
        if isinstance(backend_metadata, dict):
            if backend_metadata.get("epilogue_mode") != expected_epilogue:
            ...
            if backend_metadata.get("workspace_mode") != DIRECT_HIP_BOUNDED_SKINNY_GEMV_SMALL_N_WORKSPACE:
                self._error(
                    "direct-HIP skinny GEMV small-N captures must use "
                    f"backend_metadata.workspace_mode={DIRECT_HIP_BOUNDED_SKINNY_GEMV_SMALL_N_WORKSPACE}"
                )
            if backend_metadata.get("isa_evidence") != DIRECT_HIP_RECIPROCAL_ISA_EVIDENCE:
                self._error(
                    "direct-HIP skinny GEMV small-N captures must use "
            ...
            if isinstance(autotune_key, str):
                ...
                required_parts = {
                    "kernel": expected_kernel,
                    "epilogue": expected_epilogue,
                    "execution": "direct_hip_skinny_gemv_small_n_resident_rns",
                }
                for key, value in required_parts.items():
                    if f";{key}={value};" not in normalized_key:
                        self._error(
                            "direct-HIP skinny GEMV small-N backend_metadata.autotune_key "
                            f"must include {key}={value}"
                        )
    ...
    if self._is_vector_alu_gemv_small_n_capture():
        ...
        if self._benchmark_execution_mode() not in expected_modes:
            self._error("vector-ALU GEMV small-N captures must use a vector-GEMV small-N execution mode")
        packing = self.data.get("plan_packing")
        if isinstance(packing, dict):
            if packing.get("output_domain_name") != "native_i64_u64_current":
                self._error(
                    "vector-ALU GEMV small-N captures must declare plan_packing.output_domain_name="
                    "native_i64_u64_current"
                )
            if packing.get("output_device_current") is not True:
                self._error("vector-ALU GEMV small-N captures must leave native output current on device")

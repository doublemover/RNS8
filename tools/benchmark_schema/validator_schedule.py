...
class ValidatorScheduleMixin:
    ...
    def _validate_v4_adaptive_schedule(self, prefix: Any, schedule: Any) -> None:
        ...
        if not isinstance(selected_kernel, str) or not selected_kernel:
            ...
            finite_semantics = self.data.get("semantics") in {"finite_ring_u8", "finite_field_u8"}
            amdgpu_expected_kernel = (
                "amdgpu_builtin_cdna3_mfma_i32_16x16x32_i8_finite_u8_epilogue_v1"
                if finite_semantics
                else "amdgpu_builtin_cdna3_mfma_i32_16x16x32_i8_centered_epilogue_v1"
            )
            ...
            if isinstance(target, str) and target.startswith("gfx110"):
                ...
                amdgpu_expected_kernel = (
                    ...
                    if finite_semantics
                    else "amdgpu_builtin_cdna3_mfma_i32_32x32x16_i8_centered_epilogue_v1"
                )
            expected_kernels = {

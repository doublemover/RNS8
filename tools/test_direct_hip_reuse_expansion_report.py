...
def bounded_adaptive_capture(*, backend: str, median_us: float, pack_mode: str = "per_repeat_repack") -> dict:
    item = capture(
        backend=backend,
        median_us=median_us,
        pack_mode=pack_mode,
    ...
    item["_path"] = f"bounded-adaptive-{backend}-{pack_mode}.json"
    item["semantics"] = "bounded_u64"
    item["bound_kind"] = "global_max_abs"
    item["input_distribution"] = "adaptive-bands"
    role = {
        "prepacked_reuse_a": "stable_a_candidate",
        "prepacked_reuse_b": "stable_b_candidate",
    }.get(pack_mode, "nonreuse_baseline")
    return with_scenario_metadata(item, profile="adaptive_bounded_u64_colpair", role=role)
...
def main() -> int:
    ...
    assert summary["direct_hip_reuse_comparisons"] == 3
    assert summary["rank20_scope_comparisons"] == 3
    assert summary["candidate_workload_wins"] == 3
    assert summary["explicit_workload_selector_ready"] == 3

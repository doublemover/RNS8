...
def _strategy_ids(registry: Registry) -> list[str]:
    ...
    for index, item in enumerate(strategies):
        ...
        if not isinstance(item.get("batched_bounded_export"), bool):
            raise MetadataRegistryError(
...
def _batched_bounded_strategies(registry: Registry) -> list[str]:
    return [
        item["id"]
        for item in registry.benchmark_modes()["grouped_dispatch_execution_strategies"]
...
def _export_kernel_groups(registry: Registry) -> dict[str, list[str]]:
    groups = registry.kernels().get("export_kernel_groups")
    if not isinstance(groups, dict):
        raise MetadataRegistryError("kernels.export_kernel_groups must be an object")
    result: dict[str, list[str]] = {}
    for group, values in sorted(groups.items()):
        if not isinstance(group, str) or not group:
            raise MetadataRegistryError("kernels.export_kernel_groups keys must be nonempty strings")
        if not isinstance(values, list):
            raise MetadataRegistryError(f"kernels.export_kernel_groups.{group} must be a list")
        if not all(isinstance(item, str) and item for item in values):
            raise MetadataRegistryError(f"kernels.export_kernel_groups.{group} must contain only nonempty strings")
        _require_unique(values, f"kernels.export_kernel_groups.{group}")
        result[group] = list(values)
    return result
...
def validate_registry(registry: Registry) -> None:
    ...
    _export_kernel_groups(registry)

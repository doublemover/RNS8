...
TEST_CASE("CK fixed-prefix bounded RNS covers non-hot default moduli") {
  if (!ck_available()) {
    SKIP("CK backend is not available on this device");
  }
  ...
  for (int64_t row = 0; row < m; ++row) {
    for (int64_t col = 0; col < k; ++col) {
      A[static_cast<std::size_t>(row * k + col)] = ((row * 131 + col * 17 + 19) % 401) - 200;
    }
  }
  for (int64_t row = 0; row < k; ++row) {
    for (int64_t col = 0; col < n; ++col) {
      B[static_cast<std::size_t>(row * n + col)] = ((row * 37 + col * 113 + 23) % 307) - 153;
    }
  }
  std::vector<int64_t> cpu_out(static_cast<std::size_t>(m * n), 0);
  std::vector<int64_t> hip_out(static_cast<std::size_t>(m * n), 0);
  ...
  auto make_desc = [&](rns8_backend_kind backend) {
    auto desc = bounded_i64_desc(m, n, k, 1000000000, backend);
    desc.max_prefix = RNS8_DEFAULT_BOUNDED_PREFIX;
    desc.flags = RNS8_PLAN_FORCE_FIXED_PREFIX;
    return desc;
  };
  auto cpu_desc = make_desc(RNS8_BACKEND_CPU_REFERENCE);
  auto hip_desc = make_desc(RNS8_BACKEND_HIP_DIRECT);
  auto ck_desc = make_desc(RNS8_BACKEND_CK);
...
TEST_CASE("CK exact-wide unsigned RNS output matches CPU and direct HIP limbs") {
  if (!ck_available()) {
    SKIP("CK backend is not available on this device");
  }
  ...
  for (int64_t row = 0; row < m; ++row) {
    for (int64_t col = 0; col < k; ++col) {
      A[static_cast<std::size_t>(row * k + col)] =
          static_cast<uint64_t>((row * 257 + col * 31 + 1009) % 997 + 17);
    }
  }
  for (int64_t row = 0; row < k; ++row) {
    for (int64_t col = 0; col < n; ++col) {
      B[static_cast<std::size_t>(row * n + col)] =
          static_cast<uint64_t>((row * 43 + col * 191 + 409) % 887 + 29);
    }
  }
  constexpr uint32_t limb_count = 2;
  constexpr int64_t limb_ld = n;
  ...
  auto run_backend = [&](rns8_context* ctx, rns8_backend_kind backend) {
    ...
    REQUIRE(rns8_create_matrix(ctx, &b_desc, &b) == RNS8_SUCCESS);
    REQUIRE(rns8_create_matrix(ctx, &c_desc, &c) == RNS8_SUCCESS);
    REQUIRE(rns8_pack_u64(ctx, a, A.data(), k, 1) == RNS8_SUCCESS);
    REQUIRE(rns8_pack_u64(ctx, b, B.data(), n, 2) == RNS8_SUCCESS);
    REQUIRE(rns8_gemm_rns(ctx, plan, a, b, c, workspace) == RNS8_SUCCESS);
    std::vector<uint64_t> limbs(static_cast<std::size_t>(m * n * limb_count), 0);
...
TEST_CASE("CK finite u8 mod253 handles large generic reducer cases") {
  if (!ck_available()) {
    SKIP("CK backend is not available on this device");
  }
  ...
  constexpr uint16_t modulus = 253;
  std::vector<uint8_t> A(static_cast<std::size_t>(m * k), 0);
  std::vector<uint8_t> B(static_cast<std::size_t>(k * n), 0);
  for (int64_t row = 0; row < m; ++row) {
    for (int64_t col = 0; col < k; ++col) {
      A[static_cast<std::size_t>(row * k + col)] = static_cast<uint8_t>((row * 67 + col * 131 + 29) % modulus);
    }
  }
  for (int64_t row = 0; row < k; ++row) {
    for (int64_t col = 0; col < n; ++col) {
      B[static_cast<std::size_t>(row * n + col)] = static_cast<uint8_t>((row * 193 + col * 47 + 83) % modulus);
    }
  }
  ...
  auto cpu_desc = finite_desc(m, n, k, RNS8_FINITE_RING_U8, RNS8_BACKEND_CPU_REFERENCE, modulus);
  auto hip_desc = finite_desc(m, n, k, RNS8_FINITE_RING_U8, RNS8_BACKEND_HIP_DIRECT, modulus);
  auto ck_desc = finite_desc(m, n, k, RNS8_FINITE_RING_U8, RNS8_BACKEND_CK, modulus);

  REQUIRE(rns8_gemm_finite_ring_u8_oneshot(
              cpu, &cpu_desc, modulus, A.data(), k, B.data(), n, cpu_out.data(), n) == RNS8_SUCCESS);
  REQUIRE(rns8_gemm_finite_ring_u8_oneshot(
              hip, &hip_desc, modulus, A.data(), k, B.data(), n, hip_out.data(), n) == RNS8_SUCCESS);
  REQUIRE(rns8_gemm_finite_ring_u8_oneshot(
              ck, &ck_desc, modulus, A.data(), k, B.data(), n, ck_out.data(), n) == RNS8_SUCCESS);
  require_same_u8(cpu_out, hip_out);
  require_same_u8(cpu_out, ck_out);

...
| 2026-06-05 | large-release-validation-4096-budgeted wrap64 gate | 20260605 | 4096 strict wrap64 | Direct HIP | 295657 us median end-to-end; 3/9 byte-limb reference timed out after `--capture-timeout-seconds 300`; one-pass byte-limb reference completed at 106858000 us with checksum `13518998852724169131` | budgeted release-gate partial group; Direct HIP capture is schema-valid with required GPU events; one-pass `wrap64-byte-limb` capture is schema-valid but not a release-review timing baseline | missing a release-review byte-limb reference, so not release-reviewed, not cache eligible, and not a public 4096 wrap64 claim |
| 2026-06-05 | large-release-validation-4096-budgeted exact-wide gate | 20260605 | 4096 exact-wide signed, 4 limbs | hipBLASLt among valid GPU captures | valid GPU medians: hipBLASLt 172818 us, CK 206153 us, rocWMMA 253649 us, Direct HIP 637861 us; one-pass CPU reference completed at 112888000 us with checksum `5508849193854467465` | budgeted release-gate partial group; Direct HIP and accelerator captures are schema-valid with required GPU events; one-pass `cpu-reference` capture is schema-valid but not a release-review timing baseline | missing a release-review CPU reference, so not release-reviewed, not cache eligible, and not a public 4096 exact-wide claim |

...
| 2026-06-05 | large-release-validation-4096-budgeted wrap64 gate | 20260605 | 4096 strict wrap64 | Direct HIP | 295657 us median end-to-end; required byte-limb CPU reference timed out after `--capture-timeout-seconds 300` | budgeted release-gate partial group; Direct HIP capture is schema-valid with required GPU events; release-gate report records a timed-out required `wrap64-byte-limb` failure | missing a valid byte-limb reference, so not release-reviewed, not cache eligible, and not a public 4096 wrap64 claim |
| 2026-06-05 | large-release-validation-4096-budgeted exact-wide gate | 20260605 | 4096 exact-wide signed, 4 limbs | hipBLASLt among valid GPU captures | valid GPU medians: hipBLASLt 172818 us, CK 206153 us, rocWMMA 253649 us, Direct HIP 637861 us | budgeted release-gate partial group; Direct HIP and accelerator captures are schema-valid with required GPU events; release-gate report records a timed-out required `cpu-reference` failure after `--capture-timeout-seconds 60` | missing a valid CPU reference, so not release-reviewed, not cache eligible, and not a public 4096 exact-wide claim |

...
| Benchmark repeated-B mode | Benchmark-only reuse mode | B-side packing/setup across measured repeats | Strongest current hipBLASLt local reuse wins; rocWMMA has narrow B-cache evidence | Not AUTO-eligible because the benchmark changes the workload contract |

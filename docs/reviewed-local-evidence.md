...
| 2026-06-06 | rank 47 Direct-HIP exact-wide tree/CRT selector A/B | 20260606 | 1024 exact-wide signed, 4 limbs | Direct HIP | benchmark-only tree-pair CRT export path: 13806 us median end-to-end vs fixed-prefix Garner export at 15032 us, 1.09x faster; export event median 1475.06 us vs 3027.44 us, 2.05x faster; selected export kernel `hip_direct_export_exact_wide_signed_tree_crt_limbs_device` | same-backend selector-review captures under `temp/rank47-tree-crt-20260606/`; schema v4 valid; required GPU events available; CPU-reference comparison and `result_compare.py --allow-export-selector-diff` confirm matching shape, selected prefix, limb count, signedness, backend, target, and checksum | local Windows `gfx1100` benchmark-only implementation A/B; no README claim, installed cache entry, default route change, or Linux/CDNA claim |

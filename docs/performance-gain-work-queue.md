...
- The first rank-3/rank-49 follow-up now has actual budgeted 4096 evidence
  ...
  unattempted work. Exact-wide signed is missing a valid `cpu-reference` row
  because that required baseline timed out, and strict wrap64 is missing a
  valid `wrap64-byte-limb` row because that required reference timed out. No
  unattempted required baselines remain in those two groups, and no 4096 cache
  entry was installed.
...
| 49 | Budgeted 4096 release gate | 4096 is useful throughput classification but CPU/reference costs are high | The 4096 budget scenario has run bounded i64/u64 CPU/Direct/vector/accelerator rows, finite hot CPU/Direct/accelerator rows, exact-wide signed Direct/accelerator rows, and strict wrap64 Direct-HIP rows under `temp/perf-work-queue/large-4096-budgeted-release-current-v2/`; `tools/release_gate_report.py` schema v2 now reports 37 completed captures, 2 failed timeout rows, grouped baseline readiness, timed-out required baselines, and unattempted required baselines separately; `benchmark_sweep.py --capture-timeout-seconds` records timed-out captures as explicit `.failed.json` rows. Remaining work is chunked exact-wide CPU reference and chunked wrap64 byte-limb reference | Keep 4096 exploratory unless the budgeted release gate completes with required CPU/direct/vector baselines where applicable, required GPU events, and release-ready metadata |
...
- Review blocker for 4096 groups that lack required baselines. Implemented in
  ...
  counts. The current 4096 report has 37 completed captures plus 2 failed
  timeout rows; exact-wide signed is blocked by `cpu-reference`, and strict
  wrap64 is blocked by `wrap64-byte-limb`.

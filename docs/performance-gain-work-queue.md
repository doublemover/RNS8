...
- The remaining performance work is now the broader dispatcher and contract
  ...
  fastest independent calls, and durable workload-family wins beyond exact-wide
  signed/unsigned 64 group32.
...
- The benchmark-side descriptor contract is now explicit. The next
  ...
  pointer/slab descriptors into public or mechanically routed device-readable
  compact records: A/B/C storage offsets, leading dimensions, source versions,
  selected prefix/modulus, output policy, and checksum/export mode.
...
- Direct-HIP grouped exact-wide signed 64 path. The benchmark-owned path now
  ...
  routing the schema-validated descriptor contract through a public or internal
  dispatcher instead of benchmark-only metadata and testing broader workload
  families.

...
- Grouped-dispatch descriptor enforcement is now real internal runtime
  ...
  output, status, checksum, lifetime, and benchmark-only execution policies for
  a created plan. Public/generic grouped execution remains active.
...
| 48 | Export/reconstruction selector backend | CRT/export drives exact-wide and bounded timings, so export choice needs to be selected like a backend | Reusable internal selector and schema-visible capture fields now key CPU, Direct-HIP, vector, finite, wrap64, exact-wide, tiled, all-zero-tiled, limb-count, layout, status-policy, D2H-policy, and selected-kernel decisions; remaining gate is cache/stale-entry visibility plus compact/padded D2H and final-output/chain release A/B evidence | Keep every reconstruction/export variant inspectable and promote only setup-inclusive same-contract wins |

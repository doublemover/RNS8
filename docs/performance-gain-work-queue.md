...
|---:|---|---|---|---|
| 45 | Grouped dispatch ABI/lifetime contract | Grouped exact-wide and bounded results are the strongest branch-local wins, but the current path is still benchmark-owned | Specify and enforce descriptor ownership, descriptor reuse, strides, output domains, per-task status, per-task checksum, device-current outputs, workspace/source-version binding, and lifetime rules | Do not expose or route generic grouped dispatch until descriptor, lifetime, output, and status contracts are mechanically enforced |

...
- Current branch reconciliation: the active queue below is updated through the
  ...
  release review, finite-u8 2048 release review, generic finite-u8 2048 ring
...
| 8 | Partially completed many-small persistent/grouped workload path | Batching many 64/128/skinny exact jobs into one grouped path is likely more valuable than more isolated single-GEMM tuning | `many-small` now has a release-reviewed pre-grouped baseline matrix: seven scenario groups, 41 schema-valid captures, no missing required baselines, no duplicate backend records, and no promoted cache entries; the remaining gate is an actual grouped/persistent dispatcher with independent-call comparison, per-task correctness, setup/error aggregation, and complete event timing for promoted candidates | Keep open for grouped execution: current evidence says single-call CPU/Direct-HIP/vector baselines dominate these tiny proxies, so promote only when grouping beats independent calls including queue/setup overhead |
...
| Many-small event debt remains on non-promoted diagnostics | The `--require-events` report still fails for the small Direct-HIP one-shot resident fallback and hipBLASLt finite ring-251 64; neither is a promoted candidate, but they limit those subcaptures to diagnostic status | Add event hooks for the resident fallback one-shot path and rerun hipBLASLt finite ring-251 64 if either path becomes a promotion candidate |

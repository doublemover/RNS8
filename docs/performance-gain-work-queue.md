...
- Export/reconstruction selector work has its first reusable internal core
  ...
  behavior and mutable cache preparation point, but selector metadata is now
  testable and reusable for benchmark/cache/schema follow-through. Rank 48
  remains active for compact/padded D2H variants, schema-visible selector
  fields, cache/stale-entry integration, and release-size A/B evidence.
...
| 48 | Export/reconstruction selector backend | CRT/export drives exact-wide and bounded timings, so export choice needs to be selected like a backend | Reusable internal selector now keys CPU, Direct-HIP, vector, finite, wrap64, exact-wide, tiled, all-zero-tiled, limb-count, layout, status-policy, D2H-policy, and selected-kernel decisions; remaining gate is schema/cache/stale-entry visibility plus compact/padded D2H and final-output/chain release A/B evidence | Keep every reconstruction/export variant inspectable and promote only setup-inclusive same-contract wins |

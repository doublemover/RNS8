...
- The first rank-3/rank-49 follow-up starts that budgeted promotion path without
  running an expensive 4096 matrix yet. The disabled-by-default
  `large-release-validation-4096-budgeted` scenario now emits the full bounded
  comparator set for bounded-i64 4096, including CPU, Direct HIP, runtime vector
  ALU, hipBLASLt, CK, and rocWMMA, and the full exact-wide signed comparator set,
  including hipBLASLt. `tools/release_gate_report.py` now writes schema v2
  grouped readiness output with required baselines, missing baselines, release
  capture readiness, and blocker counts per release gate. This is enablement
  for controlled 4096 validation, not a 4096 performance claim.
...
| 49 | Budgeted 4096 release gate | 4096 is useful throughput classification but CPU/reference costs are high | The disabled-by-default 4096 budget scenario now includes bounded CPU/Direct/vector/accelerator rows and exact-wide CPU/Direct/accelerator rows; `tools/release_gate_report.py` schema v2 reports grouped baseline readiness and missing baseline blockers. Remaining work is chunked CPU reference, timeout/memory budget enforcement, and actual captures for bounded, finite, exact-wide, and wrap64 4096 shapes | Keep 4096 exploratory unless the budgeted release gate completes with required CPU/direct/vector baselines where applicable, required GPU events, and release-ready metadata |
...
- `large-release-validation-4096-budgeted` scenario with disabled-by-default
  execution. Implemented for bounded-i64 and exact-wide signed 4096 comparator
  generation: bounded now includes CPU, Direct HIP, runtime vector ALU,
  hipBLASLt, CK, and rocWMMA; exact-wide signed includes CPU, Direct HIP,
  hipBLASLt, CK, and rocWMMA.

...
- Finite-u8 generic ring 2048 now has CPU-backed release review instead of the
  ...
  864589 us. The mod-127 hipBLASLt capture remains event-incomplete and loses to
  Direct HIP, so it stays diagnostic only.
...
- The non-bounded 4096 large-shape lane now also has same-commit GPU-only
  ...
  vector captures were not included. Required-event review is mixed: 46 of 50
  captures are event-complete, all event-valid 4096 winners have required GPU
  events, and four non-promoted hipBLASLt finite captures are
  event-incomplete because the expected residue-reduce event label is missing.
  ...
  vector comparison, finite event cleanup, or public 4096 promotion.
...
| 18 | Advanced reuse/prepack workload contract promotion | Repeated-A/B wins are real, and this branch now exposes reusable-B chain, reusable consumer-B, and large-shape reusable-B scenarios; promotion needed a repeat/setup/source gate before any route could be trusted | `tools/reuse_contract_report.py` now computes setup-inclusive per-repeat time, same-backend and fastest-non-reuse speedups, break-even repeats, event availability, and prepack source-identity metadata. Current reports classify 4/12 CPU-backed 2048 repeated-B captures and 3/16 bounded 4096 exploratory repeated-B captures as workload candidates | Keep reuse out of AUTO until public workload lifetime, stale-source rejection, and chain-consumer identity are explicit; use the report as the promotion gate for explicit reuse workloads |
| 19 | hipBLASLt A/B reuse conversion from benchmark win to explicit workload contract | hipBLASLt A/B reuse is the strongest event-valid reuse signal, and hipBLASLt repeated-B remains a large-shape candidate under the new setup gate | Run `reuse_contract_report.py` on full A/B release captures with source identity and compare against same-backend plus fastest non-reuse baselines | Promote only when one-time setup, repeated-call semantics, source identity, and stale-input behavior are visible and correct |
| 20 | Advanced Direct-HIP reuse-A/reuse-B expansion beyond uniform-small bounded cases | Direct-HIP reuse now has reusable-B chain, reusable consumer-B, and large bounded scenario coverage, but reuse-A and non-bounded profiles remain thin | Existing Direct-HIP reuse captures can now be classified by the setup gate; large 2048/4096 repeated-B reports currently deprioritize Direct-HIP bounded reuse-B against fastest non-reuse baselines | Keep per-profile routing explicit; do not infer reuse from C++ type or backend alone |
...
| Large 2048/4096 captures are mixed validation evidence, not broad promotion evidence | Bounded i64/u64 2048, finite-u8 hot-modulus 2048, exact-wide signed/unsigned 2048, and strict wrap64 2048 now have CPU-backed release review; bounded/finite/exact-wide non-reuse winners are installed where AUTO cache promotion is valid, but repeated-B is still contract-limited and wrap64 is a Direct-HIP correctness path rather than cache promotion; bounded plus finite/exact-wide/wrap64 4096 now have GPU-only throughput classification, with four non-promoted hipBLASLt finite event gaps in the non-bounded matrix | Keep repeated-B as workload-contract evidence until setup identity/lifetime policy is explicit; keep 4096 claims exploratory unless a full CPU/reference release pass, runtime vector comparison, and finite event cleanup are intentionally budgeted |

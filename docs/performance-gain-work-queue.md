...
- The first rank-3/rank-49 follow-up now has actual budgeted 4096 evidence
  ...
  Bounded-i64 4096 completed CPU, Direct HIP, runtime vector ALU, hipBLASLt, CK,
  and rocWMMA release captures; hipBLASLt is fastest at 35303 us median
  end-to-end versus Direct HIP at 128995 us, runtime vector at 853232 us, and
  CPU reference at 21244300 us. The exact-wide signed 4096 GPU rows also ran
  ...
  under the new `--capture-timeout-seconds 60` budget and the group remains
  blocked by `missing_required_baselines`. No 4096 cache entry was installed.
...
| 49 | Budgeted 4096 release gate | 4096 is useful throughput classification but CPU/reference costs are high | The 4096 budget scenario has run bounded-i64 CPU/Direct/vector/accelerator rows and exact-wide signed Direct/accelerator rows under `temp/perf-work-queue/large-4096-budgeted-release-current-v2/`; `tools/release_gate_report.py` schema v2 reports grouped baseline readiness and missing baseline blockers; `benchmark_sweep.py --capture-timeout-seconds` now records timed-out captures as explicit `.failed.json` rows. Remaining work is chunked exact-wide CPU reference plus finite, bounded-u64, and wrap64 4096 gates | Keep 4096 exploratory unless the budgeted release gate completes with required CPU/direct/vector baselines where applicable, required GPU events, and release-ready metadata |
...
- `large-release-validation-4096-budgeted` scenario with disabled-by-default
  execution. Implemented and partially run: bounded-i64 4096 now has CPU,
  Direct HIP, runtime vector ALU, hipBLASLt, CK, and rocWMMA release captures;
  exact-wide signed 4096 has Direct HIP, hipBLASLt, CK, and rocWMMA captures,
  while the CPU reference timed out under a 60-second capture budget.
...
- Review blocker for 4096 groups that lack required baselines. Implemented in
  ...
  output with required baselines, missing baselines, release-capture readiness,
  and blocker counts.

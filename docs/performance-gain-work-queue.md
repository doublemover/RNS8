...
active table below now contains 40 ranks. Rank IDs are historical/stable
...
- PR #12 is merged into `main`, so the active queue has been re-ranked against
  ...
  workspace arenas, counter/resource audits, and Linux/RDNA/CDNA target gates
  now lead the table. Completed exact-wide release-matrix, finite generic
...
- Current branch reconciliation: the active queue below is updated through the
  ...
  this active queue stays focused on execution. The archive currently holds 30
  ...
  batching rank 31; large-shape matrix rank 3; and folded duplicate
...
| 48 | Export/reconstruction selector backend | CRT/export drives exact-wide and bounded timings, so export choice needs to be selected like a backend | Repo-side selector machinery now records semantic, backend, target, prefix, limb count, signedness, output layout, status policy, D2H policy, final-output/chain mode, selector key/policy, selected kernel, cache visibility, stale-entry reason, and status-elision reason in schema-visible `export_variant` metadata; `tools/export_selector_report.py` groups same-contract selector evidence. Remaining gate is real same-target compact/padded D2H, status-elision, prefix-20, and tree/CRT release A/B captures | Keep every reconstruction/export variant inspectable and promote only setup-inclusive same-contract wins |
...
| 47 | Export-bound exact-wide optimization and limb variants | Large exact-wide accelerator wins are export-bound after GEMM acceleration | Exact-wide export scenarios now cover signed/unsigned 64, 128, 512, 1024, and 2048 plus explicit fixed-limb, compact-D2H, status-elision, prefix-20 fixed-export, and tree/CRT selector candidates. Remaining gate is setup-inclusive real captures with required events, exact CPU comparison, and same requested limb contract | Promote only for the caller-requested limb contract; never substitute a narrower output claim |
...
| 59 | Advanced shape-family AUTO shadow mode | Exact-shape reviewed cache keys are too narrow for practical workloads | `tools/shape_family_shadow_report.py` now emits non-routing workload-query recommendations with selector explanations, representative reviewed-shape matrices, and hard boundaries for target, target family, semantic, signedness, finite modulus, layout, output contract, export selector, and limb count; promotion/cache reports surface these as blockers, not routing inputs | Do not route on shape-family recommendations until semantic/layout/target boundaries are mechanically enforced |
| 57 | Workspace arena implementation lane | Allocation metadata is visible but not yet an allocation-reduction mechanism | Direct-HIP workspace-owned schedule/zero-mask buffers now use an internal per-device arena with plan/target/policy identity and aligned suballocation; captures expose warmup/setup and measured-repeat allocation deltas, and `tools/resident_workspace_report.py` blocks promotion unless measured repeats are allocation-free | Promote only if allocation counters prove zero measured-repeat allocation without hiding stale workspace bugs |

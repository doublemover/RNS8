...
| HIP Graph replay is implemented as benchmark-only graph lanes, not a promoted workload contract | Rank 30 has release-size Windows `gfx1100` graph-vs-ordinary chain evidence for bounded and exact-wide 512/1024 residue-current chains, with setup-inclusive graph capture and instantiate cost included. Four rows win and four rows are deprioritized. Rank 58 closes the broader full pack/GEMM/export graph surface for bounded i64/u64, finite ring/field u8, and strict wrap64 at 512/1024. The closeout under `temp/rank58-hip-graph-full-release-20260606/` has 20 schema-v4 captures, 10 checksum-matched comparisons, no missing baselines, and release-review satisfaction for every comparison. Bounded and finite full-path graph rows are deprioritized; strict wrap64 graph replay is a local benchmark-only win at 512 and 1024 | Keep graph replay out of AUTO/cache/default routing until a public lifetime/async contract exists. Mixed hipBLASLt/CK/rocWMMA graph capture remains deferred because accelerator handles, streams, and scratch lifetimes need graph-safe contracts before measurement would be meaningful |

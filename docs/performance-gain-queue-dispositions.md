...
| HIP Graph replay is implemented as a narrow benchmark lane, not a promoted workload contract | Rank 30 has release-size Windows `gfx1100` graph-vs-ordinary chain evidence for bounded and exact-wide 512/1024 residue-current chains, with setup-inclusive graph capture and instantiate cost included. Four rows win and four rows are deprioritized, all as benchmark-only classifications | Keep graph replay out of AUTO/cache/default routing until a public lifetime/async contract exists; broader graph coverage remains gated by active rank 58 |

...
| Native vector-ALU | required on HIP builds | bounded i64/u64 only | yes for bounded i64/u64 | source-level 192-bit-limb accumulator, no matrix-engine instructions claimed | bounded-u64 release groups were previously blocked by the same vector baseline; public runtime backend now exists but still needs reviewed release promotion before durable AUTO selection | selector candidate wired behind reviewed-cache promotion |
...
| Wrap64 matrix-engine | current direct-HIP byte-limb path is production baseline; rocWMMA candidate is internal | no public backend | Windows `gfx1100` private differentials cover single-cell K tails, exact 16x16x16 tiles, padded carry-heavy tails, ragged two-tile output, 64x64x64 and 128x128x128 exhaustive CPU-oracle full-output cases, 512x512x512 and 1024x1024x1024 full candidate-vs-direct-HIP output checks with sampled CPU-oracle cells, and the `k=32768` accepted / `k=32769` rejected boundary; benchmark smoke also checks same-seed 64x64x64 CPU/direct-HIP/candidate `checksum_u64` parity | rocWMMA candidate is covered by the `v_wmma` no-divide/no-INT32-global-store gate | direct-HIP v3 remains measured release GPU path; candidate-inclusive seed-20260603 release review showed candidate medians 4825/5202/37481/264657 us versus direct-HIP 3653/1852/9430/41237 us at 64/128/512/1024, so no wrap64 matrix-engine win exists | not durable |
...
- Optimized strict `mod 2^64` GPU byte GEMMs, accelerator integration of the
  ...
  tiles, padded carry-heavy tails, ragged two-tile output, 64x64x64 and
  128x128x128 exhaustive CPU-oracle full-output cases, 512x512x512 and
  1024x1024x1024 full candidate-vs-direct-HIP output checks with sampled
  CPU-oracle cells, and the `k=32768` accepted / `k=32769` rejected boundary,
  plus ISA-gated WMMA evidence and a 64x64x64 same-seed benchmark checksum
  smoke against CPU byte-limb and direct-HIP captures. A candidate-inclusive
  release review at seed `20260603` matched checksums but lost to direct HIP at
  every 64/128/512/1024 shape, so public backend integration is blocked until a
  materially faster kernel exists; any promotion-required exhaustive 512/1024
...
- Optimized finite-field algorithms beyond the explicit-modulus
  correctness-grade CPU/direct-HIP finite path. Narrow direct-HIP reducers now
  cover modulus 251 with byte folding from `256 == 5 (mod 251)`, modulus 255

...
differentials, and the current `k=32768` accepted / `k=32769` rejected
boundary. The benchmark smoke also runs same-seed 64x64x64 CPU byte-limb,
direct-HIP, and rocWMMA-candidate captures and requires matching `checksum_u64`
values. It is not a public backend, not selected by AUTO, and not release
...
512/1024 correctness evidence beyond the 64x64x64 and 128x128x128 full-output
differentials, and reviewed release captures proving it beats direct-HIP v3
before it can displace the current production GPU path.
